 ...
 */
static
enum mad_error IRAM_ATTR_FUN III_sideinfo(struct mad_bitptr *ptr, unsigned int nch,
 ...
 */
static
unsigned int IRAM_ATTR_FUN III_scalefactors_lsf(struct mad_bitptr *ptr,
 ...
 */
static
unsigned int IRAM_ATTR_FUN III_scalefactors(struct mad_bitptr *ptr, struct channel *channel,
			      struct channel const *gr0ch, unsigned int scfsi)
{
  ...
  slen1 = unalChar((const char *)&sflen_table[channel->scalefac_compress].slen1);
  slen2 = unalChar((const char *)&sflen_table[channel->scalefac_compress].slen2);
 ...
 */
static
void IRAM_ATTR_FUN III_exponents(struct channel const *channel,
		   unsigned char const *sfbwidth, signed int exponents[39])
{
  ...
  if (channel->block_type == 2) {
	...
	l += unalChar((const char *)&sfbwidth[sfbi++]);
      }
    }
	...
	(signed int) (channel->scalefac[sfbi + 2] << scalefac_multiplier);
      ...
      l    += 3 * unalChar((const char *)&sfbwidth[sfbi]);
      sfbi += 3;
    }
  }
  else {  /* channel->block_type != 2 */
 ...
 */
static
enum mad_error IRAM_ATTR_FUN III_huffdecode(struct mad_bitptr *ptr, mad_fixed_t xr[576],
			      struct channel *channel,
			      ...
			      unsigned int part2_length)
{
  ...
  /* big_values */
  {
    ...
    sfbound = xrptr + unalChar((const char *)sfbwidth++);
    ...
    while (big_values-- && cachesz + bits_left > 0) {
      ...
      if (xrptr == sfbound) {
	sfbound += unalChar((const char *)sfbwidth++);
 ...
 */
static
void IRAM_ATTR_FUN III_reorder(mad_fixed_t xr[576], struct channel const *channel,
		 unsigned char const sfbwidth[39])
{
  ...
  if (channel->flags & mixed_block_flag) {
    ...
    l = 0;
    while (l < 36)
      l += unalChar((const char *)sfbwidth++);
  }
  ...
  f = unalChar((const char *)sfbwidth++);
  w = 0;

  for (l = 18 * sb; l < 576; ++l) {
    if (f-- == 0) {
      f = unalChar((const char *)sfbwidth++) - 1;
      w = (w + 1) % 3;
    }
 ...
 */
static
enum mad_error IRAM_ATTR_FUN III_stereo(mad_fixed_t xr[2][576],
			  ...
			  unsigned char const *sfbwidth)
{
  ...
  if (header->mode_extension & I_STEREO) {
	...
	while (l < 36) {
	  n = unalChar((const char *)&sfbwidth[sfbi++]);
 ...
 */
static
void IRAM_ATTR_FUN III_aliasreduce(mad_fixed_t xr[576], int lines)
{
...
#  if 1
static
void IRAM_ATTR_FUN fastsdct(mad_fixed_t const x[9], mad_fixed_t y[18])
{
...
static inline
void IRAM_ATTR_FUN sdctII(mad_fixed_t const x[18], mad_fixed_t X[18])
{
  mad_fixed_t tmp[9];
  int i;
...
static inline
void IRAM_ATTR_FUN dctIV(mad_fixed_t const y[18], mad_fixed_t X[18])
{
  mad_fixed_t tmp[18];
  int i;
 ...
 */
static inline
void IRAM_ATTR_FUN imdct36(mad_fixed_t const x[18], mad_fixed_t y[36])
{
  mad_fixed_t tmp[18];
  int i;
 ...
 */
static
void IRAM_ATTR_FUN III_imdct_l(mad_fixed_t const X[18], mad_fixed_t z[36],
 ...
 */
static
void IRAM_ATTR_FUN III_imdct_s(mad_fixed_t const X[18], mad_fixed_t z[36])
{
 ...
 */
static
void IRAM_ATTR_FUN III_overlap(mad_fixed_t const output[36], mad_fixed_t overlap[18],
 ...
 */
static inline
void IRAM_ATTR_FUN III_overlap_z(mad_fixed_t overlap[18],
		   mad_fixed_t sample[18][32], unsigned int sb)
{
  unsigned int i;
 ...
 */
static
void IRAM_ATTR_FUN III_freqinver(mad_fixed_t sample[18][32], unsigned int sb)
{
  unsigned int i;
 ...
 */
static
enum mad_error IRAM_ATTR_FUN III_decode(struct mad_bitptr *ptr, struct mad_frame *frame,
 ...
 */
int IRAM_ATTR_FUN mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  struct mad_header *header = &frame->header;
