...
void C610_GetMotorInfo(can_message_t* RxMsg)
{
	...
	if(abs(MotorInfo[1].Current) > 1000){		//When detecting that the motor current exceeds 1A, immediately stop the motor
		C610_SendCurrentVal(0, 0, 0, 0);
		vTaskDelay(100);
	}
	// ESP_LOGI("MotorInfo", "ESC_ID: %d, AnglePulse: %d, Velocity: %d, Current: %d", 
	// 						ESC_Id,
